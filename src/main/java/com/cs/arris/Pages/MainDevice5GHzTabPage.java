...
//	public MobileElement seekBar;
	...
	public boolean verifyUIOn5GHzDevicePage() {
		utils.log().info("                                            ");
		utils.log().info("********************************************");
		utils.log().info("Details of UI Elements on the 5GHz Tab Page ");
		utils.log().info("********************************************");
		try {
...
//	}
		...
		try {
			...
			try {
				if(txtDevices.equals(connDevices)) {
					...
					utils.log().info("Count of devices displayed on the main Router Image is equal to the count of 5GHz devices connected to the main Router");
				}else {
					utils.log().info("Count of devices displayed on the main Router Image is not equal to the count of 5GHz devices connected to the main Router");
				}
			}catch(Exception e) {}
	...
	@Override
	public boolean isAt() {
		if (fiveGhzTab.isDisplayed() && fiveGhzTab.getText().equals("5.0 GHz")) {
			utils.log().info("On MAIN Router - 5.0 GHz Tab  Page ");
			return true;
		} else {
